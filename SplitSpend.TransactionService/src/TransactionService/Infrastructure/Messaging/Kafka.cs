...
/// <summary>
/// Shared base for all Kafka consumers: manual commit, 3-attempt retry with back-off.
/// </summary>
    ...
    where TMessage : class
{
    ...
    protected KafkaConsumerBase(
        IConfiguration config, ILogger log, string topic, string groupId)
    {
        Logger = log;
        _topic = topic;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    Logger.LogError(ex, "Deserialize failed for {Topic}", _topic);
                ...
                for (int attempt = 1; attempt <= 3 && !success; attempt++)
                {
                    ...
                    {
                        Logger.LogWarning(ex, "Attempt {A}/3 failed for {Topic}. Retrying…",
                            attempt, _topic);
                        await Task.Delay(500 * attempt, stoppingToken);
                    }
                    catch (Exception ex)
                    {
                        Logger.LogError(ex, "Permanently failed after 3 attempts for {Topic}", _topic);
                    }
                }
